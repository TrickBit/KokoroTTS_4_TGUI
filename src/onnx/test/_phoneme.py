...
def normalize_text(text):
    ...
    for a, b in zip('ï¼Œã€‚ï¼ï¼Œï¼šï¼›ï¼Ÿ', ',.!,:;?'):
    ...
    text = re.sub(r'(?i)[$Â£]\d+(?:\.\d+)?(?: hundred| thousand| (?:[bm]|tr)illion)*\b|[$Â£]\d+\.\d\d?\b', flip_money, text)
...
def get_vocab():
    ...
    vocab_mapping = {
        ';': 1, ':': 2, ',': 3, '.': 4, '!': 5, '?': 6, 'â€”': 9, 'â€¦': 10,
        '"': 11, '(': 12, ')': 13, '"': 14, '"': 15, ' ': 16, 'Ìƒ': 17,
        'Ê£': 18, 'Ê¥': 19, 'Ê¦': 20, 'Ê¨': 21, 'áµ': 22, 'ê­§': 23, 'A': 24,
        'I': 25, 'O': 31, 'Q': 33, 'S': 35, 'T': 36, 'W': 39, 'Y': 41,
        'áµŠ': 42, 'a': 43, 'b': 44, 'c': 45, 'd': 46, 'e': 47, 'f': 48,
        'h': 50, 'i': 51, 'j': 52, 'k': 53, 'l': 54, 'm': 55, 'n': 56,
        'o': 57, 'p': 58, 'q': 59, 'r': 60, 's': 61, 't': 62, 'u': 63,
        'v': 64, 'w': 65, 'x': 66, 'y': 67, 'z': 68, 'É‘': 69, 'É': 70,
        'É’': 71, 'Ã¦': 72, 'Î²': 75, 'É”': 76, 'É•': 77, 'Ã§': 78, 'É–': 80,
        'Ã°': 81, 'Ê¤': 82, 'É™': 83, 'Éš': 85, 'É›': 86, 'Éœ': 87, 'ÉŸ': 90,
        'É¡': 92, 'É¥': 99, 'É¨': 101, 'Éª': 102, 'Ê': 103, 'É¯': 110,
        'É°': 111, 'Å‹': 112, 'É³': 113, 'É²': 114, 'É´': 115, 'Ã¸': 116,
        'É¸': 118, 'Î¸': 119, 'Å“': 120, 'É¹': 123, 'É¾': 125, 'É»': 126,
        'Ê': 128, 'É½': 129, 'Ê‚': 130, 'Êƒ': 131, 'Êˆ': 132, 'Ê§': 133,
        'ÊŠ': 135, 'Ê‹': 136, 'ÊŒ': 138, 'É£': 139, 'É¤': 140, 'Ï‡': 142,
        'ÊŽ': 143, 'Ê’': 147, 'Ê”': 148, 'Ëˆ': 156, 'ËŒ': 157, 'Ë': 158,
        'Ê°': 162, 'Ê²': 164, 'â†“': 169, 'â†’': 171, 'â†—': 172, 'â†˜': 173, 'áµ»': 177
    }
...
def phonemize(text, lang, norm=True):
    ...
    ps = re.sub(r'(?<=[a-zÉ¹ËŒ])(?=hËˆÊŒndÉ¹Éªd)', ' ', ps)
    ps = re.sub(r' z(?=[;:,.!?Â¡Â¿â€”â€¦"Â«Â»"" ]|$)', 'z', ps)
    if lang == 'a':
        ps = re.sub(r'(?<=nËˆaÉªn)ti(?!ËŒ)', 'di', ps)
